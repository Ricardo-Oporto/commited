import { formatChangeList, formatImpactSection, resolveBranchPrefix, sanitizeBodyLine, summarizeChanges } from './templates';
...
export function generateCommitMessage(options: GenerateCommitMessageOptions): string {
  const title = buildTitle(options);
  const body = buildBody(options);
  return [title, body].filter((section) => section.length > 0).join('\n\n');
}

function buildTitle({ branchName, branchPrefixPattern, changes }: GenerateCommitMessageOptions): string {
...
function buildBody({ changes, includeImpactSection }: GenerateCommitMessageOptions): string {
  const summaryLine = sanitizeBodyLine(`- ${capitalizeFirst(summarizeChanges(changes))}.`);
